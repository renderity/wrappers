...
namespace RDTY::WRAPPERS
{
	...
	void Scene::addObjects (std::vector<Object*> _objects)
	{
		...
		for (Object* object : _objects)
		{
			...
			object->scene_index_data_size = object->index_data.size() * sizeof(uint32_t);

			object->scene_vertex_data_offset = vertex_data_size;
			object->scene_vertex_data_size = object->position_data.size() * sizeof(float);

			index_data_size += object->scene_index_data_size;
			vertex_data_size += object->scene_vertex_data_size;
		}

		index_data.resize(index_data_size / sizeof(__m128i));
		position_data.resize(vertex_data_size / sizeof(__m128));
		normal_data.resize(vertex_data_size / sizeof(__m128));

		cout << "size" << endl;
		cout << index_data.size() * 16 << endl;
		...
		for (Object* object : _objects)
		{
			...
			cout << "object" << endl;
			cout << object->index_data[0] << endl;
			cout << object->index_data[1] << endl;
			cout << object->index_data[2] << endl;
			cout << object->index_data[4] << endl;

			cout << index_data.size() * 16 << endl;
			cout << object->index_data.size() * 4 << endl;
			cout << object->scene_index_data_size << endl;
			...
			for (size_t i {}; i < object->scene_index_data_size / 4; ++i)
			{
				index_data[i][0] = object->index_data[i * 4 + 0];
				index_data[i][1] = object->index_data[i * 4 + 1];
				index_data[i][2] = object->index_data[i * 4 + 2];
				index_data[i][3] = object->index_data[i * 4 + 3];
			}

			// memcpy(index_data.data() + object->scene_index_data_offset / 16, object->index_data.data(), object->scene_index_data_size);
			// memcpy(index_data.data(), f.data(), 16);

			cout << "index_data" << endl;
			cout << index_data[0][0] << endl;
			cout << index_data[0][1] << endl;
			cout << index_data[0][2] << endl;
			cout << index_data[0][3] << endl;

			cout << "_________" << endl;

			return;

			std::vector<__m128> vec {};
			vec.resize(1);

			memcpy(vec.data(), f.data(), 16);
			...
			{
				size_t offset { object->scene_vertex_data_offset / sizeof(__m128) };

				for (size_t i { object->scene_index_data_offset / sizeof(__m128i) }, i_max { i + (object->scene_index_data_size / sizeof(__m128i)) }; i < i_max; ++i)
				{
			...
			memcpy(position_data.data() + object->scene_vertex_data_offset / 16, object->position_data.data(), object->scene_vertex_data_size);
			memcpy(normal_data.data() + object->scene_vertex_data_offset / 16, object->normal_data.data(), object->scene_vertex_data_size);
	...
	inline bool testPointInsideBox (float* point, float* min, float* max)
	{
		return
		(
	...
	inline bool testPointInsideBox (const __m128& point, const __m128& min, const __m128& max)
	{
		return
		(
			point[0] <= max[0] && point[0] >= min[0] &&
	...
	inline float testRayBoxIntersectionSimd (const __m128& ray_origin, const __m128& ray_direction, const __m128& box_min, const __m128& box_max)
	{
		...
		__m128 v = _mm_sub_ps(_mm_add_ps(_mm_mul_ps(ray_direction, _mm_set1_ps(t_near)), ray_origin), ray_origin);

		float dot = (ray_direction[0] * v[0]) + (ray_direction[1] * v[1]) + (ray_direction[2] * v[2]);

		return (dot > 0.0f);
	}

	// bool Scene::testTriangleSimd (const size_t& triangle_index, const __m128& min, const __m128& max, PSIMD* p)
	bool Scene::testTriangleSimd (const __m128i& vertex_indices, const __m128& min, const __m128& max, PSIMD* p)
	{
		cout << "position_data" << position_data.size() << endl;
		cout << vertex_indices[0] << endl;
		p->p1 = position_data[vertex_indices[0]];
		// p->p2 = position_data[vertex_indices[1]];
		// p->p3 = position_data[vertex_indices[2]];
		...
		// // TODO: case where surface of triangle intersects box.
		// if
		// (
		// 	// point inside box
	...
	void Scene::testSimd (Object* object)
	{
		...
		size_t triangles_index { (sizeof(triangles) / sizeof(__m128i)) / object_length * object_index };

		triangles_index += ((triangles_index % sizeof(__m128i)) == 0 ? 0 : (sizeof(__m128i) - (triangles_index % sizeof(__m128i))));

		const size_t dimension_segment_count { object->dimension_segment_count };

		const __m128 step { _mm_set1_ps((object->bounding_box_max[0] - object->bounding_box_min[0]) / dimension_segment_count) };

		memcpy(boxes[box_index_bounding].min, object->bounding_box_min, 16);

		boxes[box_index_bounding].triangle_start = dimension_segment_count;

		memcpy(boxes[box_index_bounding].max, object->bounding_box_max, 16);
		...
		for (size_t x {}; x < dimension_segment_count; ++x)
		{
			for (size_t y {}; y < dimension_segment_count; ++y)
			{
				...
				{
					const __m128 xyz = _mm_set_ps(x, y, z, 0.0f);

					_min = _mm_add_ps(_mm_load_ps(object->bounding_box_min), _mm_mul_ps(step, xyz));
					_max = _mm_add_ps(_min, step);



					const size_t triangle_start { triangles_index };

					for (uint32_t i { object->scene_index_data_offset / 4 }, i_max { i + (object->scene_index_data_size / 4) }; i < i_max; ++i)
					{
						cout << "testSimd" << endl;
						cout << i << endl;
						cout << index_data[i][0] << endl;
						cout << index_data[i][1] << endl;
						cout << index_data[i][2] << endl;
						if (testTriangleSimd(index_data[i], _min, _max, &p))
						{
							triangles[triangles_index++] = index_data[i];
						}
					}

					const size_t triangle_end { triangles_index };
			...
			for (size_t x {}; x < dimension_segment_count; ++x)
			{
				for (size_t y {}; y < dimension_segment_count; ++y)
				{
					for (size_t z {}; z < dimension_segment_count; ++z)
					{
						...
						for (uint32_t i { object->scene_index_data_offset }, i_max { object->scene_index_data_offset + object->scene_index_data_size }; i < i_max; i += 4)
						{
